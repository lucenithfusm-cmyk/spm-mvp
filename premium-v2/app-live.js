...
function renderMap(){
 ...
 $('safetyBox').innerHTML=m.urgent.length?'<div class="notice danger"><b>Prioridad de seguridad.</b> Tus respuestas incluyen una señal que requiere atención médica urgente antes de continuar.</div>':m.review.length?'<div class="notice warn"><b>Revisión profesional recomendada.</b> Hay datos que conviene revisar externamente mientras usas solo módulos seguros.</div>':'<div class="notice good"><b>En tus respuestas no aparecieron señales principales de seguridad que impidan continuar con esta orientación educativa.</b> Esto no sustituye una valoración médica.</div>';
...
async function saveCoach(){
 ...
 if(error){msg(error.message,'danger');return}S.checkins.push(data);$('coachDecision').innerHTML=`<div class="notice ${d.code==='professional_review'?'danger':'good'}"><b>Daily Coach sugiere:</b> ${d.text}</div>`;renderProgress();await maybeWeeklyReview(v.day);msg('Check-in guardado. Tu progreso ya está persistido.','good');
}
