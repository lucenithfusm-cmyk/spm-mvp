(()=>{
'use strict';
if(window.SPM_RECOVERY_MEDIA_LOCAL_V4)return;
window.SPM_RECOVERY_MEDIA_LOCAL_V4=true;
...
function hydrate(){
 document.querySelectorAll('.spm-v4').forEach(root=>{
  ...
  root.querySelectorAll('.spm-v4-card.doctor .spm-v4-visual').forEach(box=>{
   let v=box.querySelector('video'),needsLoad=false;
   if(!v){v=document.createElement('video');box.insertBefore(v,box.firstChild);needsLoad=true}
   if(!String(v.getAttribute('src')||'').includes('dr-spm-performance-anxiety-content.mp4')){v.src=DOCTOR;needsLoad=true}
