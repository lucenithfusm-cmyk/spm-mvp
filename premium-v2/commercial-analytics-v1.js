...
function bind(){const gate=document.querySelector('.spmGate');if(!gate||gate.dataset.analytics==='1')return;gate.dataset.analytics='1';save('offer_view',gate);const start=gate.querySelector('.startPay'),unlock=gate.querySelector('.pilotUnlock'),back=gate.querySelector('.backResult');if(start)start.addEventListener('click',()=>{save('cta_create_program',gate);save('checkout_view',gate)});if(back)back.addEventListener('click',()=>save('checkout_back',gate));if(unlock)unlock.addEventListener('click',()=>save('pilot_unlock',gate))}
