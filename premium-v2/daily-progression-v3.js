...
function renderHero(){
 ...
 $('#spmGoToday',hero)?.addEventListener('click',()=>focusToday(true));
}
function selectPhase(){const t=phaseFor(currentDay),tabs=$$('.phaseBtn');if(tabs[t-1]&&!tabs[t-1].classList.contains('on'))tabs[t-1].click()}
function focusToday(scroll=false){selectPhase();setTimeout(()=>{const cards=$$('.dayCard');cards.forEach(c=>{c.classList.remove('spm-current-day');$('.spm-current-chip',c)?.remove()});const card=cards.find(c=>Number($('.dayNum',c)?.textContent||0)===currentDay);if(!card)return;card.classList.add('spm-current-day');const top=$('.dayTop',card);if(top&&!$('.spm-current-chip',top))top.insertAdjacentHTML('beforeend','<span class="spm-current-chip">HOY</span>');if(!card.classList.contains('open'))top?.click();if(scroll)card.scrollIntoView({behavior:'smooth',block:'start'})},120)}
function sync(){
 const c=calendarDay(),f=completedFloor();currentDay=clamp(Math.max(c,f));const s=loadState()||{};if(currentDay>(s.day||1)){s.day=currentDay;saveState(s)}
 window.SPM_CURRENT_DAY=currentDay;document.documentElement.dataset.spmCurrentDay=String(currentDay);renderHero();focusToday(false)
}
...
let pending=false;new MutationObserver(()=>{if(pending)return;pending=true;requestAnimationFrame(()=>{pending=false;if($('#dayGrid'))sync()})}).observe(document.body,{childList:true,subtree:true});
document.addEventListener('click',e=>{if(e.target.closest('#navPlan,#goPlan,.doneBtn'))setTimeout(sync,350)});document.addEventListener('visibilitychange',()=>{if(!document.hidden)sync()});document.addEventListener('DOMContentLoaded',()=>setTimeout(sync,700),{once:true});setTimeout(sync,1400);
})();