(()=>{
'use strict';
const KEY='spm_free_assessment_v1',RECEIPT='spm_free_assessment_receipt_v1';
const $=s=>document.querySelector(s);
const lang=()=>window.SPM_LANGUAGE?.get?.()||'es';
const t=(es,en)=>lang()==='en'?en:es;
const esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
let api=null,draft=null,result=null,stage='landing',busy=false,storageAvailable=true;
function read(key){try{return JSON.parse(sessionStorage.getItem(key)||'null')}catch{return null}}
function write(key,value){try{sessionStorage.setItem(key,JSON.stringify(value))}catch{storageAvailable=false}}
function clear(){draft=null;result=null;try{sessionStorage.removeItem(KEY);sessionStorage.removeItem(RECEIPT)}catch{}}
const priorities={
...
function temporaryNote(){return storageAvailable?t('Sin cuenta, puedes retomar tu evaluación mientras esta pestaña conserve sus datos. No se guarda todavía en tu cuenta.','Without an account, you can resume while this tab retains its data. It is not yet saved to your account.'):t('Tu navegador no permite guardar este recorrido temporalmente. Mantén esta página abierta para continuar.','Your browser cannot save this journey temporarily. Keep this page open to continue.')}
...
function openOffer(){if(!result||result.urgent.length)return;stage='offer';window.SPM_COMMERCIAL_GATE_PREVIEW.open()}
function renderAccount(mode='signup'){
 ...
 $('#spmGuestAccountForm').onsubmit=async e=>{
  ...
  try{
   ...
   status.textContent=t('Guardando tu evaluación…','Saving your assessment…');await api.save(read(RECEIPT)||{});renderSaved();
...
window.SPM_GUEST_ENTRY={
 ...
 ready(handle){api=handle;const saved=read(KEY);draft=saved?.version===1?saved:null;translateChrome();renderLanding()},
 progress(value){if(draft&&JSON.stringify([draft.motives,draft.answers])!==JSON.stringify([value.motives,value.answers])){try{sessionStorage.removeItem(RECEIPT)}catch{}}draft=value;write(KEY,value)},
 receipt(value){write(RECEIPT,value)},
 complete(value){result=value;window.SPM_INSIGHTS_PREVIEW?.close();window.SPM_COMMERCIAL_PREVIEW?.close();renderResult()}
};
