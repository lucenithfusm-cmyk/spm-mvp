(()=>{'use strict';
if(window.SPM_RECOVERY_AUDIO_CONTROLLER_V7)return;window.SPM_RECOVERY_AUDIO_CONTROLLER_V7=true;
/* Single owner for Recovery story audio: all four turns are queued from the user's tap.
...
const MALE=/jorge|diego|carlos|enrique|juan|miguel|alvaro|álvaro|alejandro|antonio|pablo|andres|andrés|mateo|martin|martín|daniel|alex|aaron|arthur|fred|ralph|tom|oliver|james|gordon|lee|eddy|reed|rocko|sandy|shelley|grandpa|wavenet-[bdj]|neural2-[bcj]/i;
const FEMALE=/monica|mónica|paulina|luciana|sofia|sofía|carmen|helena|isabel|laura|maria|maría|salome|salomé|samantha|victoria|karen|moira|tessa|fiona|ava|allison|susan|serena|kate|siri|shelley|grandma|wavenet-[acefgh]|neural2-[adefghi]/i;
function voices(r){
 ...
 const patient=local.find(v=>MALE.test(v.name))||local.find(v=>!FEMALE.test(v.name))||null;
 /* Keep Doctor SPM on a female voice in the active language and never deliberately reuse it for the patient. */
 const doctor=local.find(v=>FEMALE.test(v.name)&&v!==patient)||local.find(v=>v!==patient)||local[0]||null;
...
function stop(){
 ...
 if(root){root.querySelector('.spm-v4-wave')?.classList.remove('playing');resetPauseButton(root)}root=null;
}
...
function resetPauseButton(r){const b=r?.querySelector('.spm-v4-pause');if(b)b.textContent='⏸ '+(isEn(r)?'Pause audio':'Pausar audio')}
function play(r){
 if(!synth){status(r,'',isEn(r)?'Audio is not available in this browser.':'El audio no está disponible en este navegador.');return}
 stop();root=r;resetPauseButton(r);const token=run,cards=[...r.querySelectorAll('.spm-v4-card')];
 ...
 cards.forEach((card,i)=>{
   ...
   u.rate=doctor?.92:.95;u.pitch=doctor?1.04:.78;
